...
static constexpr int kBoardNumInputs = 6;
static constexpr int kBoardMaxPorts = 6;
#elif defined(CONFIG_BOARD_KC868_A8)
static constexpr int kBoardNumInputs = 8;
static constexpr int kBoardMaxPorts = 8;
#else
static constexpr int kBoardNumInputs = 16;
static constexpr int kBoardMaxPorts = 8;
#endif

// On single-radio boards (everything except the A16) only Radio A exists. Force
// the operation mode so a stored or imported dual-radio config can never actuate
// a nonexistent Radio B. No-op on the A16, where all three modes are valid.
static radio_operation_mode_t board_clamp_radio_mode(const radio_operation_mode_t mode) {
    ...
    if (mode != RADIO_OP_MODE_SINGLE_A) {
        ESP_LOGW(TAG, "This board is single-radio only; forcing radio_operation_mode to SINGLE_A (was %d)", mode);
...
// Clamp antenna-ports-per-radio to the board's relay count (A6=6, A8/A16=8) so a
// stored or imported config can't select a port with no relay behind it.
static uint8_t board_clamp_num_ports(const uint8_t n) {
    if (n > kBoardMaxPorts) {
        ESP_LOGW(TAG, "num_antenna_ports %u exceeds this board's max of %d; clamping", n, kBoardMaxPorts);
        return static_cast<uint8_t>(kBoardMaxPorts);
    }
    return n;
}
...
esp_err_t ConfigManager::update_config(const antenna_switch_config_t &new_config) {
    ...
    antenna_switch_config_t updated_config;
    {
        ...
        *current_config_ = new_config;
        ...
        current_config_->num_antenna_ports = board_clamp_num_ports(current_config_->num_antenna_ports);
...
esp_err_t ConfigManager::load_from_nvs() const {
    ...
    if (base_load_err == ESP_OK) {
        if (base_data_size == sizeof(base_nvs_config_data_t)) {
            ...
            current_config_->num_antenna_ports = board_clamp_num_ports(loaded_base_data.num_antenna_ports);
